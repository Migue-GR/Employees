...
import androidx.lifecycle.liveData
import com.employees.model.enums.ErrorCode.SOMETHING_WENT_WRONG
import com.employees.utils.AppSession
import com.employees.utils.EmployeesException
import com.employees.utils.UseCaseResult
...
fun <T : Any> resultLiveData(work: suspend () -> T?) = liveData {
    ...
    } catch (e: EmployeesException) {
        Timber.e(e)
        emit(UseCaseResult.Error(e))
    } catch (t: Throwable) {
        Timber.e(t)
        emit(UseCaseResult.Error(EmployeesException(SOMETHING_WENT_WRONG)))
    }
}
...
fun <T : Any> LiveData<UseCaseResult<T>>.observeWith(
    ...
    onError: (error: EmployeesException) -> Unit = {}
